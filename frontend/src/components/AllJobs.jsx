...
const AllJobs = () => {
  ...
  const {text,setText,loc,setLoc,type,setType,show,salMin,salMax} = useContext(context);
  useEffect(()=>{
    const getData = async () =>{
      try{
  ...
  const newData = jobs.filter((ele) =>{
    ...
    return ele.Location.includes(loc) && ele.jobType.includes(type) && ele.salaryMax >= salMin && ele.salaryMax <= salMax;
  })
  return (
